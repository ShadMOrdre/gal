...
--function gal.register_liquid(source, flowing, itemname, bucket_item, claypot_item, inventory_image, name, bucket_name, claypot_name, groups, force_renew)
function gal.register_liquid(source, flowing, bucket_item, bucket_wood_item, bucket_steel_item, claypot_item, inv_image, bucket_name, bucket_wood_name, bucket_steel_name, claypot_name, groups, force_renew)
		...
		gal.liquids[flowing] = gal.liquids[source]
...
minetest.register_craftitem(":gal:tool_bucket_empty", {
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		local liquiddef = gal.liquids[node.name]
				...
				for id,fluid in pairs(buffers) do
					if fluid ~= "" then
						...
						if took.millibuckets == 1000 and took.name == fluid then
							if gal.liquids[fluid] then
								itemstack = ItemStack(gal.liquids[fluid].bucket_item)
								if ndef.on_timer then
			...
			if #buffers > 0 then
				for id,fluid in pairs(buffers) do
					if fluid ~= "" then
						...
						if took.millibuckets == 1000 and took.name == fluid then
							if gal.liquids[fluid] then
								itemstack = ItemStack(gal.liquids[fluid].bucket_wood_item)
								if ndef.on_timer then
									minetest.get_node_timer(lpos):start(ndef.node_timer_seconds or 1.0)
								end
								break
			...
			if #buffers > 0 then
				for id,fluid in pairs(buffers) do
					if fluid ~= "" then
						...
						if took.millibuckets == 1000 and took.name == fluid then
							if gal.liquids[fluid] then
								itemstack = ItemStack(gal.liquids[fluid].bucket_steel_item)
								if ndef.on_timer then
									minetest.get_node_timer(lpos):start(ndef.node_timer_seconds or 1.0)
								end
								break
			...
			if #buffers > 0 then
				for id,fluid in pairs(buffers) do
					if fluid ~= "" then
						...
						if took.millibuckets == 1000 and took.name == fluid then
							if gal.liquids[fluid] then
								itemstack = ItemStack(gal.liquids[fluid].claypot_item)
								if ndef.on_timer then
									minetest.get_node_timer(lpos):start(ndef.node_timer_seconds or 1.0)
								end
								break
...
for i, liq in ipairs(gal.lib.csv.read("|", gal.path .. "/gal_geology_containers.csv")) do
	...
	--minetest.log(S("[MOD] gal:  dump gal.liquids: " .. dump(gal.liquids)))

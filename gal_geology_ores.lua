...
--local mg_seed = gal.mapgen.mg_seed

for i, ore in ipairs(gal.lib.csv.read("|", gal.path .. "/" .. gal.ore_data_file .. ".csv")) do
		...
		if oretype ~= "" then
			...
			if oretype == "sheet" then
				...
				minetest.register_ore({
					...
					y_min = tonumber(ymin) - gal.mapgen.biome_vertical_blend,
					y_max = tonumber(ymax) + gal.mapgen.biome_vertical_blend,
					...
					column_height_min = tonumber(col_hght_min) or (gal.mapgen.biome_vertical_range / 2) + gal.mapgen.biome_vertical_blend,
					column_height_max = tonumber(col_hght_max) or gal.mapgen.biome_vertical_range + (gal.mapgen.biome_vertical_blend * 2),
			...
			if oretype == "stratum" then
				...
				minetest.register_ore({
					...
					wherein = "gal:" .. wherein,
					clust_scarcity = 1,
					y_min = gal.mapgen.maxheight_beach,
					y_max = (gal.mapgen.maxheight_highland - 10),

...
function gal.fluid_short(str)
	return string.lower(str):gsub("%s", "_")
end

function gal.vessels.get_is_vessel(stack)
	return minetest.get_item_group(stack:get_name(), "vessel") > 0
end

function gal.vessels.get_is_empty_vessel(stack)
	return minetest.get_item_group(stack:get_name(), "vessel_blank") > 0
end

function gal.vessels.get_vessel_contents(stack)
	if not gal.vessels.get_is_vessel(stack) then return nil end
...
local function update_vessel(stack)
	...
	local meta = stack:get_meta()
	local contents, fluid_name, capacity = gal.vessels.get_vessel_contents(stack)
...
function gal.vessels.take_fluid(stack, amount)
	if not gal.vessels.get_is_vessel(stack) then return nil end
...
function gal.vessels.register_vessel(data)
...
--		})
	...
	if data.fluids then
		...
		for i, v in pairs(gal.liquids) do
...
fluidity = {}
fluidity.fluid_short = gal.fluid_short

fluidity.florbs = {}
fluidity.florbs.get_is_vessel = gal.vessels.get_is_vessel
fluidity.florbs.get_is_empty_vessel = gal.vessels.get_is_empty_vessel
fluidity.florbs.get_vessel_contents = gal.vessels.get_vessel_contents
fluidity.florbs.add_fluid = gal.vessels.add_fluid
fluidity.florbs.take_fluid = gal.vessels.take_fluid
fluidity.florbs.register_vessel = gal.vessels.register_vessel
